...
LINEUP_REQ = {"QB": 1, "WR": 3, "RB": 2, "TE": 1}  # K/DEF optional late
SUPERFLEX_POS = {"QB","WR","RB","TE"}

# --------- STARTER DATA (works immediately) ---------
# You can add more any time in the Data tab or via CSV upload.
available_seed = [
    # VETS (FA = from your EXT list)
...
df_players["Rank"] = (
    ...
    + df_players.index  # deterministic order within groups
)
# Superflex QB boost (approx ~25%)
...
# ---------- League Rosters (skeleton you can extend) ----------
with tabs[1]:
...
with tabs[2]:
    ...
    with col1:
        team = st.selectbox("Team drafting", st.session_state.teams, index=8)  # default to your team
    with col2:
...
# ---------- Data (paste more players / upload CSV) ----------
with tabs[3]:
    ...
    if st.button("Add pasted rows"):
        ...
        if rows:
            ...
            extra["Rank"] = (
                extra["Position"].map({"QB":1,"WR":2,"RB":3,"TE":4}).fillna(9)*100 + range(len(extra))
            )
